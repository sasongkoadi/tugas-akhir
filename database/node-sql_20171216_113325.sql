...
-- CREATE VIEW "historyorderservice" -----------------------
-- CREATE VIEW "historyorderservice" ---------------------------
CREATE ALGORITHM = UNDEFINED DEFINER = `root`@`%` SQL SECURITY DEFINER VIEW `historyorderservice`
AS select `node-sql`.`orderservice`.`id` AS `id`,`node-sql`.`user`.`id` AS `UserId`,`node-sql`.`user`.`namaDepan` AS `namaDepan`,`node-sql`.`user`.`namaBelakang` AS `namaBelakang`,`node-sql`.`user`.`userName` AS `username`,`node-sql`.`user`.`alamatUser` AS `alamatUser`,`node-sql`.`user`.`nomerPonselUser` AS `nomerPonselUser`,`node-sql`.`motor`.`namaMotor` AS `namaMotor`,`node-sql`.`motor`.`jenisMotor` AS `jenisMotor`,`node-sql`.`motor`.`CC` AS `CC`,`node-sql`.`pemilikmotor`.`platNomer` AS `platNomer`,`node-sql`.`pemilikmotor`.`nomerRangka` AS `nomerRangka`,`node-sql`.`pemilikmotor`.`nomerMesin` AS `nomerMesin`,`node-sql`.`orderservice`.`jenisPerbaikan` AS `jenisPerbaikan`,`node-sql`.`orderservice`.`tanggalService` AS `tanggalService`,`node-sql`.`orderservice`.`jamService` AS `jamService` from (((`node-sql`.`pemilikmotor` join `node-sql`.`user` on((`node-sql`.`pemilikmotor`.`namaPemilik` = `node-sql`.`user`.`id`))) join `node-sql`.`motor` on((`node-sql`.`pemilikmotor`.`motorPemilik` = `node-sql`.`motor`.`id`))) join `node-sql`.`orderservice` on((`node-sql`.`orderservice`.`motorPelanggan` = `node-sql`.`pemilikmotor`.`id`)));
-- -------------------------------------------------------------
-- ---------------------------------------------------------


-- CREATE VIEW "historyservice" ----------------------------
-- CREATE VIEW "historyservice" --------------------------------
CREATE ALGORITHM = UNDEFINED DEFINER = `root`@`%` SQL SECURITY DEFINER VIEW `historyservice`
AS select `node-sql`.`dataorderservice`.`id` AS `id`,`node-sql`.`user`.`id` AS `UserId`,`node-sql`.`user`.`namaDepan` AS `NamaPelanggan`,`node-sql`.`user`.`userName` AS `username`,`node-sql`.`user`.`alamatUser` AS `Alamat`,`node-sql`.`user`.`nomerPonselUser` AS `NomerHp`,`node-sql`.`motor`.`namaMotor` AS `Motor`,`node-sql`.`motor`.`jenisMotor` AS `Jenis`,`node-sql`.`motor`.`CC` AS `CC`,`node-sql`.`pemilikmotor`.`platNomer` AS `Plat`,`node-sql`.`pemilikmotor`.`nomerRangka` AS `nomerRangka`,`node-sql`.`pemilikmotor`.`nomerMesin` AS `nomerMesin`,`node-sql`.`orderservice`.`jenisPerbaikan` AS `jenisPerbaikan`,`node-sql`.`orderservice`.`tanggalService` AS `tanggalService`,`node-sql`.`orderservice`.`jamService` AS `jamService`,`node-sql`.`dataorderservice`.`DataPerbaikan` AS `DataPerbaikan`,`node-sql`.`pegawai`.`nama_depan` AS `namaMekanik`,`node-sql`.`pegawai`.`NIK` AS `NIK`,`node-sql`.`dataorderservice`.`serviceBerikutnya` AS `serviceBerikutnya` from (((((`node-sql`.`pemilikmotor` join `node-sql`.`user` on((`node-sql`.`pemilikmotor`.`namaPemilik` = `node-sql`.`user`.`id`))) join `node-sql`.`motor` on((`node-sql`.`pemilikmotor`.`motorPemilik` = `node-sql`.`motor`.`id`))) join `node-sql`.`orderservice` on((`node-sql`.`orderservice`.`motorPelanggan` = `node-sql`.`pemilikmotor`.`id`))) join `node-sql`.`dataorderservice` on((`node-sql`.`dataorderservice`.`serviceOrderId` = `node-sql`.`orderservice`.`id`))) join `node-sql`.`pegawai` on((`node-sql`.`dataorderservice`.`PegawaiId` = `node-sql`.`pegawai`.`pegawaiId`)));
-- -------------------------------------------------------------
-- ---------------------------------------------------------


-- CREATE VIEW "pemilikroda2" ------------------------------
-- CREATE VIEW "pemilikroda2" ----------------------------------
CREATE ALGORITHM = UNDEFINED DEFINER = `root`@`%` SQL SECURITY DEFINER VIEW `pemilikroda2`
AS select `node-sql`.`pemilikmotor`.`id` AS `id`,`node-sql`.`user`.`id` AS `UserId`,`node-sql`.`user`.`namaDepan` AS `namaDepan`,`node-sql`.`user`.`namaBelakang` AS `namaBelakang`,`node-sql`.`user`.`userName` AS `username`,`node-sql`.`user`.`alamatUser` AS `alamatUser`,`node-sql`.`user`.`nomerPonselUser` AS `nomerPonselUser`,`node-sql`.`motor`.`namaMotor` AS `namaMotor`,`node-sql`.`motor`.`jenisMotor` AS `jenisMotor`,`node-sql`.`motor`.`CC` AS `CC`,`node-sql`.`pemilikmotor`.`platNomer` AS `platNomer`,`node-sql`.`pemilikmotor`.`nomerRangka` AS `nomerRangka`,`node-sql`.`pemilikmotor`.`nomerMesin` AS `nomerMesin` from ((`node-sql`.`pemilikmotor` left join `node-sql`.`user` on((`node-sql`.`pemilikmotor`.`namaPemilik` = `node-sql`.`user`.`id`))) left join `node-sql`.`motor` on((`node-sql`.`pemilikmotor`.`motorPemilik` = `node-sql`.`motor`.`id`)));
-- -------------------------------------------------------------
-- ---------------------------------------------------------
